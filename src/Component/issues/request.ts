...
import  {Name2Header} from "./config";


const baseUrl = 'https://oj.cs.sdu.edu.cn:8889/imapi';
...
const get: any = async (url: string, name:string,params?: object, config?: AxiosRequestConfig) => {
    return await dealResponse(service.get(url, {
        ...{headers: {"Cache-Control": "no-cache, no-store, must-revalidate","SDUOJUserInfo":Name2Header[name]}}, params, ...config,
    }), url)
}

const post: any = async (url: string,name:string, data: object, config?: AxiosRequestConfig) => {
    return await dealResponse(service.post(url, data, {
        ...{headers: {"Cache-Control": "no-cache, no-store, must-revalidate","SDUOJUserInfo":Name2Header[name]}}, ...config
    }), url);
}
